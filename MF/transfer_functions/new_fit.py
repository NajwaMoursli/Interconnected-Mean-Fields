...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 16:35:08 2019
...
def pseq_params(params):
    ...
    if 'P' in params.keys(): return Qe,Te, Ee, Qi, Ti, Ei, Gl, Cm, El, params['Ntot'], params['pconnec'], params['gei'], P0, P1, P2, P3, P4, P5, P6, P7, P8, P9, P10
    else : return Qe,Te, Ee, Qi, Ti, Ei, Gl, Cm, El, params['Ntot'], params['pconnec'], params['gei'], P0
# @numba.jit()
...
def Nu_out2(muV, sV, TvN, Vthre, Gl, Cm):
    return 1/(5e-3+2*TvN*Cm/Gl/(sp_spec.erfc((Vthre-muV)/np.sqrt(2)/sV)))
...
def effective_Vthre2(Y, muV, sV, TvN, Gl, Cm):
    Vthre_eff = muV+np.sqrt(2)*sV*sp_spec.erfcinv(\
                    2.*TvN*Cm/Gl/(1/Y-5e-3)) # effective threshold
    return Vthre_eff
...
def TF_my_templateup2(fe, fi,XX, Qe, Te, Ee, Qi, Ti, Ei, Gl, Cm, El, Ntot, pconnec, gei, P0, P1, P2, P3, P4, P5, P6, P7, P8, P9, P10):
    # here TOTAL (sum over synapses) excitatory and inhibitory input
    ...
    Fout_th = Nu_out2(muV, sV, TvN, Vthre, Gl, Cm)
    if(hasattr(Fout_th, "__len__")):
        #print("ttt",isinstance(muV, list), hasattr(muV, "__len__"))
        Fout_th[Fout_th<1e-8]=1e-8
...
def TF_my_templateup_heterogeneity_effective_p(fe, fi,XX, Qe, Te, Ee, Qi, Ti, Ei, Gl, Cm, El, Ntot, pconnec, gei, P0, P1, P2, P3, P4, P5, P6, P7, P8, P9, P10):
    # here TOTAL (sum over synapses) excitatory and inhibitory input
    ...
    def Phet(k):
        locale=gaussian(k,1.,0.1)*TF_my_templateup(fe, k*fi,XX, Qe, Te, Ee, Qi, Ti, Ei, Gl, Cm, El, Ntot, pconnec, gei, P0, P1, P2, P3, P4, P5, P6, P7, P8, P9, P10) ## 0.1 because 1/sqrt(100)
        return locale


    
    outhet, err = quad(Phet, 0, 2)
...
def fitting_Vthre_then_Fout2(Fout, Fe_eff, fiSim,w, params,\
                               maxiter=50000, xtol=1e-5, with_square_terms=False):
    ...
    Vthre_eff = effective_Vthre2(Fout[i_non_zeros], muV[i_non_zeros],\
                sV[i_non_zeros], TvN[i_non_zeros], params['Gl'], params['Cm'])
    ...
    def Res(p):
        if not with_square_terms:
            ...
            params['P'] = p
        return np.mean((Fout-\
                        TF_my_templateup2(Fe_eff, fiSim,w, *pseq_params(params)))**2)
    ...
    diff=(TF_my_templateup2(Fe_eff, fiSim,w, *pseq_params(params))-Fout).mean()
    diff_M=(TF_my_templateup2(Fe_eff, fiSim,w, *pseq_params(params))-Fout).max()
    print("rrrrr",diff,diff_M)
          
          
  
    plt.plot(fiSim,Fout,'rd',fiSim,TF_my_templateup2(Fe_eff, fiSim,w, *pseq_params(params)),'bs')
    #plt.plot(fiSim,Fe-eff,Fout,'rd')
    plt.show()
    thrplot=threshold_func(muV, sV,TvN, muGn, *(plsq.x))
...
def make_fit_from_data(DATA, with_square_terms=False):
    ...
    ### NEW VERSION ####
    for r_ps, d_ps, f_ps in os.walk(DATA):
        f_ps=f_ps
    
    print(DATA)
    
    '''MEANfreq=np.zeros((int(numb_e),int(numb_i)))
    SDfreq=np.zeros((int(numb_e),int(numb_i)))
    w=np.zeros((int(numb_e),int(numb_i)))
    fe=np.zeros((int(numb_e),int(numb_i)))
    fi=np.zeros((int(numb_e),int(numb_i)))'''
    ...
    for fname in f_ps:
        ...
        fe_loc,fi_loc,MEANfreq_loc, SDfreq_loc,\
        params,w_loc = np.load(filename,allow_pickle=True,encoding="latin1")
        fe.append(fe_loc)
        fi.append(fi_loc)
        w.append(w_loc)
        SDfreq.append(SDfreq_loc)
        MEANfreq.append(MEANfreq_loc)
    
    MEANfreq=np.array(MEANfreq)
    SDfreq=np.array(SDfreq)
    w=np.array(w)
    fe=np.array(fe)
    fi=np.array(fi) 
    ...
    P = fitting_Vthre_then_Fout(MEANfreq, fe, fi,w, params,\
...
#    plt.plot(Fe_eff[2,:],MEANfreq[2,:],"bs",fiSim[:,5],MEANfreq[:,5],"o")
    ...
    # then we save it:
    filename = DATA+ '_fit.npy'
    print ('coefficients saved in ', filename)
    np.save(filename, np.array(P))

    return P

def make_fit_from_data2(DATA, with_square_terms=False):
    ...
    for fe_inp in np.arange(0,fe_max,0.5):
        for fi_inp in np.arange(0,fi_max,0.5):
    
                filename=DATA+'/fe_'+str(fe_inp)+'_fi_'+str(fi_inp)+'.npy'
                inde=int(fe_inp*2)
                indi=int(fi_inp*2)
                fe[inde,indi],fi[inde,indi],MEANfreq[inde,indi], SDfreq[inde,indi],\
                params,w[inde,indi] = np.load(filename)#,allow_pickle=True,encoding="latin1")

    P = fitting_Vthre_then_Fout2(MEANfreq, fe, fi,w, params,\
                                with_square_terms=with_square_terms)
    ...
    # then we save it:
    filename = DATA.replace('.npy', '2_fit.npy')
    print ('coefficients saved in ', filename)
    np.save(filename, np.array(P))
