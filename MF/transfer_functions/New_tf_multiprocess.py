    ...
    for i in range(fiSim.size):
        ...
        while (e<JUMP.size):
            ...
            if (vec[0]>JUMP[e-1]): # if we make a too big jump
                ...
                if verbose:
                    print (" exc :", e+1," over ",feSim.size)
                    print (" inh :", i+1," over ",fiSim.size)
...
if __name__=='__main__':
    ...
    parser.add_argument("--fe_inp",type=float,
                         help="input excitatory frequency")
    
    parser.add_argument("--fi_inp",type=float,
                         help="input inhibitory frequency")
    ...
    if args.save:
        FILE = 'data/'+args.Neuron_Model+'_'+args.Network_Model#+'/fe_'+str(args.fe_inp)+'_fi_'+str(args.fi_inp)+'.npy'
    else:
        FILE = 'data/example_data.npy'
        
    
    def multi_TF(filename=FILE,dt=args.dt,tstop=args.tstop,verbose=True,\
                 fe_in=args.fe_inp,fi_in=args.fi_inp,SEED=args.SEED):
    
        print('fe = ', fe_in, '  fi = ', fi_in , '\n \n')
    
        FILE_save = filename+'/fe_'+str(fe_in)+'_fi_'+str(fi_in)+'.npy'
        #filename=FILE
        #dt=args.dt
        #tstop=args.tstop
        #verbose=True
        ...
        vec[0],wec[0]= single_experiment(t,\
            fe_in*(1-params['gei'])*params['pconnec']*params['Ntot'],
            fi_in*params['gei']*params['pconnec']*params['Ntot'], params, seed=0)
    
       
        '''
       Here we do not consider adaptive step
       '''
        ...
        for seed in range(1,SEED):
            ...
            vec[seed],wec[seed] = single_experiment(t,\
                    fe_in*(1-params['gei'])*params['pconnec']*params['Ntot'],\
                    fi_in*params['gei']*params['pconnec']*params['Ntot'], params, seed=seed)
            if verbose:
                 print ("== ---- _____________ seed :",seed)
        MEANfreq = vec.mean()
        SDfreq = vec.std()
       ...
       # print 'RRRRRRRRRRRRRRRRRRRRRRRRRR', vec.mean(),wec.mean()
        ...
        # then we save the results
        np.save(FILE_save, np.array([fe_in,fi_in,MEANfreq, SDfreq, params,w]))
        print ('numerical TF data saved in :', FILE_save)
        print ('The value of W is', w)
...
fe_list=np.linspace(0,int(args.fe_inp),2*int(args.fe_inp)+1)
fi_list=np.linspace(0,int(args.fi_inp),2*int(args.fi_inp)+1)

input_list=[]

for i in range(len(fe_list)):
    for j in range(len(fi_list)):
        input_list.append([fe_list[i],fi_list[j]])

print(input_list)

# Step 2: `pool.apply` the `howmany_within_range()`
for par in input_list: print(FILE,args.dt,args.tstop,True,*par,args.SEED)

#results = [pool.apply(multi_TF, args=(args.dt,args.tstop,True,*par,20)) for par in input_list]

for par in input_list: pool.apply(multi_TF, args=(FILE,args.dt,args.tstop,True,*par,args.SEED))
