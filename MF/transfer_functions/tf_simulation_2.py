...
def adexp_sim(t, I, Ge, Gi,
    ...
    return V, np.array(spikes),waver/wcounting,np.mean(Vc),wpi/wcounting
...
def single_experiment(t, fe, fi, params, seed=0):
    ...
    I = np.zeros(len(t))
    v, spikes,w,Vaver,partex = adexp_sim(t, I, ge, gi, *pseq_adexp(params))
...
def generate_transfer_function(params,\
    ...
    for i in range(fiSim.size):
        ...
        for e in range(feSim.size):
            ...
            SDfreq[i][e] = vec.std()
            w[i][e]= wec.mean()
            print 'RRRRRRRRRRRRRRRRRRRRRRRRRR',i,e,Fe_eff[i][e],fiSim[i], vec.mean(),wec.mean()
