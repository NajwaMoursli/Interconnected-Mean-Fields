...
''' Here we postulate that the heterogeneity in connectivity will only affect input FR and relative weight in the total. As the corresponding subpopulations are averaged, there is no need of keeping track of all the individual input/output weights. That should do the trick.'''
...
def make_fit_from_data(DATA, with_square_terms=False):
    ...
    MEANfreq, SDfreq, Fe_eff, fiSim, params,w = np.load(DATA,allow_pickle=True,encoding="latin1")
    print(params)
    print(params.keys())
...
def make_fit_from_data2(DATA, with_square_terms=False):
    ...
    P = fitting_Vthre_then_Fout2(Fout, Fe_eff, fiSim,w, params,\
                                with_square_terms=with_square_terms)
