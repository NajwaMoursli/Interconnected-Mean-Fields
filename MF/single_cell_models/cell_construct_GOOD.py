...
def get_membrane_equation(neuron_params, synaptic_array,\
    ...
    if neuron_params['delta_v']==0:
        ...
        eqs = """
        dV/dt = (%(Gl)f*nS*(%(El)f*mV*hetEl - V) + %(Gl)f*nS*%(delta_v)f*mV*exp(-(%(Vthre)f*mV-V)/(%(delta_v)f*mV)) + I - w_adapt+%(ampnoise)f*pA*xi*((%(Gl)f*nS)/(%(Cm)f*pF))**-0.5)/(%(Cm)f*pF) : volt (unless refractory)
